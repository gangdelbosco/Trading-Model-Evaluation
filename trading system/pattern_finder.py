...
def combination (data_frame, ind_1, ind_2, ind_3, ind_4, ind_5, ind_6, ind_7, ind_8, ind_9, ind_10, ind_11, ind_12, ind_13, ind_14, ind_15, ind_16, ind_17, ind_18, ind_19, ind_20, ind_21, ind_22, ind_23, ind_24, ind_25, ind_26, ind_27, ind_28, ind_29, ind_30, ind_31, ind_32, ind_33, change_index,  n_confirm, n_indicator, n_conbination):
    data_frame['confirm'] = False
    combinations = []
    # Iterate for the desired number of combinations
    for i in range(n_conbination):
    ...
    array_combinations = []
    full_combo = []
    for combination in combinations: #serie di numeri in array di serie
        array_combination = []
        val_tcross = []
        i = 0
        
        for number in combination: #numeri in serie di numeri
            array_combination.append(array_mapping[number]) #valori indicatore
            val_tcross.append(change_index[number-1]) #valore da superare indicatore
            name_iter.append(name_mapping[number]) #stringa nome indicatore
            i+=1
        
        array_combinations.append(array_combination) #array di colonne valori indicatori
        val_cross.append(val_tcross) # array valori da superare per ogni indicatore
        for i in range(len(array_combinations)):
            combino = combinations[i]
            df = data_frame.copy()
            for j in range(n_indicator):

                surname= name_mapping[combino[j]]
                val_perc = change_index[combino[j]-1]
                array_map = array_combinations[i]
                df = sign_model(data_frame, array_map[j], val_perc, surname)
                full_combo.append(df) #? colonne contenenti dataframe
    
    profit_array_ =[]
    ratio_array_ = []

    
    for j in range(len(full_combo)): 
        df_i =full_combo[j] #scelta dataframe
        df_i['final_confirm']= 0
        open_pos = []
        close_pos = []
        profit_array =[]
        ratio_array = []
        win_percent = []
        for i in range(len(df_i)-7):
            for k in range(n_indicator):
                if df_i[name_iter[k]][i] == 1: #se colonna di ogni indicatore contiene conferma la colonna della conferma finale riceve+1
                    df_i['final_confirm'][i]+=1
                    if df_i['final_confirm'][i]==n_confirm: #se n_conferme per trigger buy viene raggiunto segna posizione di apertura e chiusura
                        open_pos.append(df_i['Close'][i])
                        close_pos.append(df_i['Close'][i+7])
        
        for i in range(len(close_pos)): #per ogni trade chiuso calcola il profitto
        ...
        profit_array_.append(profit_array)
        ratio_array_.append(ratio_array)

    
    return array_combinations, profit_array_, ratio_array_
...
def generate_combinations(n):
    ...
    for i in range(n):
        ...
        combination = random.sample(range(0, 32), x)
        combinations.append(combination)
    return combinations